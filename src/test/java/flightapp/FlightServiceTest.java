...
public class FlightServiceTest {
  ...
  /**
   * The Java property name which specifies the pathname and filename
   * at which FlightServiceTest will log its list of passing tests.
   */
  private static final String TESTLOG_PROPNAME = "flightapp.report_file";
   ...
   * <code>flightapp.show_private_output</code>.  Needs to be specified globally and not
   * via dbconn.properties.
   */
   ...
   * should be displayed, or merely the summary (ie, test name + success/fail).  It can only
   * be read from the command line or similar; we explicitly do NOT parse it out of
   * dbconn.properties, lest students submit an override dbconn.properties to Gradescope.
   */
  private static final String PRIVATE_OUTPUT_PROPNAME = "flightapp.show_private_output";

  /**
   * The Java property name used to specify which FlightServiceTest-specific test cases
   * should be run.  The input can be a list, with its items separated by colons (:).
   * If a list item is a directory name, FlightServiceTest will run every file with a
   * suffix of TEST_FILESUFFIX in that directory.
   */
  private static final String TEST_CASE_PROPNAME = "flightapp.testcases";
  ...
  static List<Session> parse(String filename) throws IOException, SQLException {
    ...
    while ((l = reader.readLine()) != null) {
      ...
      // Record an alternate result for the current session
        ...
        l = l.split(COMMENT, 2)[0].replaceAll("\\s+$", "");
  ...
  public static void setup() {
    ...
    String reportPath = System.getProperty(TESTLOG_PROPNAME);
    if (reportPath != null) {
  ...
  public static Collection<String> files() throws IOException {
    String pathString = System.getProperty(TEST_CASE_PROPNAME, "cases/");
  ...
  public void runTest() throws Exception {
    ...
    try {
      ...
      // For each output session, see if it matches an expected scenario.
