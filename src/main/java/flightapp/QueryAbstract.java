...
public abstract class QueryAbstract {
  ...
  + "WHERE state = 'active' AND backend_xid IS NOT NULL AND pid = pg_backend_pid()";
   ...
   */
  public final String create(String username, String password, int initAmount) {
    try {
      // TODO(hctang): rename to transaction_create(), for consistency with other methods
   ...
   *
   * The results are sorted based on total flight duration.
   *
   * @param originCity
   ...
   *
   *         Itinerary [itinerary number]: [number of flights] flight(s), [total flight duration] minutes\n
  ...
  *
   ...
   *
   *         Reservation [reservation ID] (unpaid/paid)[depending on true or false]:\n
